...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans text-gray-900 antialiased">
        @php
            ...
            // For a super admin, nav links follow whichever organization they've
            // navigated into via the URL (which may not be one they belong to, and
            // may differ from their currentOrganization) rather than currentOrganization.
            $navOrganization = auth()->user()->isSuperAdmin() && $routeOrganization instanceof \App\Models\Organization
                ? $routeOrganization
            ...
            >
                <div class="flex h-16 w-full flex-none items-center gap-2 px-4 lg:justify-center">
                    <a href="{{ route('dashboard') }}" wire:navigate>
                        <x-application-logo class="text-lg" :dark="$isPlatformAdmin" />
                    </a>
                ...
                <div class="overflow-y-auto">
                    <div class="w-full p-4">
                        <nav class="space-y-1 {{ $isPlatformAdmin ? 'rounded-lg bg-white p-2 shadow-xs' : '' }}">
                            ...
                            @if ($navOrganization)
                                <x-nav-link :href="route('organizations.people.index', $navOrganization)" :active="request()->routeIs('organizations.people.*')" wire:navigate>
                                    <span class="flex flex-none items-center text-gray-400">
                                        <svg class="inline-block size-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                                        </svg>
                                    </span>
                                    <span class="grow">{{ __('People') }}</span>
                                </x-nav-link>

                                <x-nav-link :href="route('organizations.products', $navOrganization)" :active="request()->routeIs('organizations.products')" wire:navigate>
                                    <span class="flex flex-none items-center text-gray-400">
                                        <svg class="inline-block size-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.481 1.243-1.15 1.243H5.38a1.125 1.125 0 01-1.15-1.243l1.263-12A1.125 1.125 0 016.617 7.5h10.766c.576 0 1.059.435 1.119 1.007z" />
                                        </svg>
                                    </span>
                                    <span class="grow">{{ __('Products') }}</span>
                                </x-nav-link>

                                <x-nav-link :href="route('organizations.transactions', $navOrganization)" :active="request()->routeIs('organizations.transactions')" wire:navigate>
                                    <span class="flex flex-none items-center text-gray-400">
                                        <svg class="inline-block size-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                                        </svg>
                                    </span>
                                    <span class="grow">{{ __('Transactions') }}</span>
                                </x-nav-link>

                                <x-nav-link :href="route('organizations.events', $navOrganization)" :active="request()->routeIs('organizations.events')" wire:navigate>
                                    <span class="flex flex-none items-center text-gray-400">
                                        <svg class="inline-block size-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
                                        </svg>
                                    </span>
                                    <span class="grow">{{ __('Events') }}</span>
                                </x-nav-link>
                            @endif
                        </nav>
                    </div>
